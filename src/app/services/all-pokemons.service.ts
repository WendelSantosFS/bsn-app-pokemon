...
export class AllPokemons {

      public pokemons = [
        {"count":1302,"next":"https://pokeapi.co/api/v2/pokemon?offset=60&limit=30","previous":"https://pokeapi.co/api/v2/pokemon?offset=0&limit=30",
            "results":[
    ...
    getPoke (name: string | null) {
        if (typeof(name) === null) return null;

        const poke = this.pokemons[0].results
        const pokeFind = poke.find( (pok) => pok.name === name)
        return pokeFind
    }
